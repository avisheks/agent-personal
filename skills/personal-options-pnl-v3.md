# Options PnL Summarizer + Wheel Trade Advisor — Operating Instructions (v3)
...
1. **PnL Reporting** (inherited from v2): Computes realized PnL for Cash-Secured Puts (CSP) and Covered Calls (CC) across three brokerage accounts (Fidelity, Tastytrade, Thinkorswim). Produces a consolidated report with monthly/weekly breakdowns, trendline charts, and trade-level detail.

2. **Wheel Trade Recommendations** (new in v3): Generates forward-looking trade suggestions for the next period based on the same sector/ticker universe, using a multi-agent evaluation framework that prioritizes risk-adjusted total return over raw premium.
...
├── skills/
│   └── personal-options-pnl-v3.md     # This file
└── .local/options-pnl/
    ├── inp/
...
# Part 2: Wheel Trade Recommendations

## Overview

After generating the PnL report (Part 1), the system produces forward-looking trade recommendations using the same ticker/sector universe defined in `sector-map.json`. The recommendations follow a multi-agent evaluation framework designed to maximize **risk-adjusted total return**, NOT maximum premium.

**Key Principle:** When uncertain, recommend NO TRADE. Cash is a valid position.

## Trading Universe

The stock universe is defined by the same `sector-map.json` config file used for PnL reporting. Only tickers in this file are eligible for trade recommendations. Do not introduce new securities without user approval.

## Recommendation Pipeline

```
PnL Report (v2 output) → Historical Performance Analysis → Multi-Agent Evaluation → Trade Scoring → Recommendation Output
```

The recommendation engine runs AFTER the PnL report is generated and uses the report data as input for historical context.

## Multi-Agent Evaluation Framework

Each candidate trade is evaluated by multiple independent "agents" (analysis modules). All agents must be satisfied before a trade is recommended.

### Agent 1: Fundamental Suitability

For each stock, evaluate:
- Revenue growth, earnings growth, free cash flow
- Balance sheet strength, competitive position
- Valuation relative to peers and history
- Business cyclicality and recent deterioration

**Score: 0–100**

Apply the "Would I want to own this?" test:
- Would the stock remain attractive after a -10%, -20%, -30%, -40%, -50% decline?
- If the fundamental thesis would be damaged by a normal correction, REJECT the trade

**Hard veto:** Do NOT recommend a CSP merely because the premium is attractive.

### Agent 2: Market / Technical Analysis

Analyze:
- Trend (20/50/100/200-day MAs)
- Relative strength, support/resistance
- Momentum, recent drawdown
- Volume, volatility regime

Classify the setup: Strong Bullish / Bullish / Neutral / Bearish / Strong Bearish

Do not use technical analysis alone to justify a trade.

### Agent 3: Options Evaluation

For candidate CSPs, evaluate multiple combinations (not just highest premium):
- Strike, expiration, DTE, delta
- Implied volatility, IV rank/percentile
- Premium, premium/capital-at-risk ratio
- Break-even price, probability of expiring OTM
- Earnings exposure, early assignment risk
- Liquidity (bid/ask spread, open interest)

**Normal parameters (guidelines, not hard constraints):**
- Delta: 20–30
- DTE: 20–45
- High liquidity required
- Favorable IV environment preferred

**Reject:** Options with poor liquidity or wide bid/ask spreads.

### Agent 4: Earnings & Catalyst Risk

Identify upcoming:
- Earnings dates
- Product announcements, regulatory events
- Major economic exposures, litigation, M&A
- Binary catalysts

**Rule:** Do NOT sell a CSP through earnings merely because premium is attractive. If earnings exposure is accepted, explicitly document why.

### Agent 5: Portfolio Risk

Before approving any trade, calculate:
- Total portfolio delta, portfolio beta
- Sector exposure, single-stock exposure
- Cash utilization, maximum potential assignment exposure
- Correlation between positions

**Stress scenarios (run for every candidate):**
- Market -10%, -20%, -30%
- Candidate stock -40%, -50%

**Rule:** Never approve a CSP simply because sufficient cash technically exists.

### Agent 6: Adversarial Review

For every proposed trade, generate a deliberate bear case:
- Hidden concentration risk
- Overvaluation signals
- Fundamental deterioration
- Upcoming catalyst the market is pricing
- Volatility mispricing (premium is high because risk IS high)
- Correlation with existing holdings
- False confidence from recent win streaks

**The adversarial agent CAN veto a trade.**

### Agent 7: Opportunity Cost

For every proposed trade, explicitly compare against:
- Holding cash (risk-free rate)
- Buying the underlying stock directly
- Different strike or expiration
- Bull put spread (defined risk)
- Doing nothing

Answer: "Why is this trade better than keeping the cash?"
...
| Weight | Factor |
|--------|--------|
| 25% | Fundamental quality |
| 20% | Valuation attractiveness |
| 15% | Technical setup |
| 15% | Options attractiveness (premium/risk) |
| 10% | Portfolio diversification benefit |
| 10% | Downside protection |
| 5% | Liquidity |

**Hard vetoes (override any score):**
- Fundamental quality unacceptable
- Liquidity poor
- Earnings risk unacceptable
- Portfolio concentration exceeded
- Assignment would breach risk limits
- User would not genuinely want to own the stock

## Risk Limits

Never breach without explicit user approval:

| Limit | Description |
|-------|-------------|
| MAX_SINGLE_STOCK_EXPOSURE | Maximum % of portfolio in one name |
| MAX_SECTOR_EXPOSURE | Maximum % in one sector |
| MAX_CASH_DEPLOYMENT | Maximum % of cash deployed in options |
| MINIMUM_CASH_RESERVE | Minimum % kept as dry powder |
| MAX_ASSIGNMENT_EXPOSURE | Maximum total $ if all CSPs assigned |
| MAX_PORTFOLIO_DELTA | Maximum net portfolio delta |
| MAX_LOSS_PER_TRADE | Maximum loss on any single trade |

**Default values (user should override in config):**
```json
{
  "MAX_SINGLE_STOCK_EXPOSURE_PCT": 10,
  "MAX_SECTOR_EXPOSURE_PCT": 30,
  "MAX_CASH_DEPLOYMENT_PCT": 70,
  "MINIMUM_CASH_RESERVE_PCT": 30,
  "MAX_ASSIGNMENT_EXPOSURE_PCT": 80,
  "MAX_PORTFOLIO_DELTA": 50,
  "MAX_LOSS_PER_TRADE_PCT": 5
}
```

Store in `.local/data/options-pnl/risk-limits.json`.

## Covered Call Agent

When shares are assigned, DO NOT automatically sell a CC.

First answer: "Would I genuinely be happy selling these shares at the proposed strike?"

Evaluate:
- Current stock price vs cost basis
- Unrealized P&L
- Fundamental valuation, technical trend
- IV environment
- Expected upside remaining
- Probability of assignment at proposed strike

If stock appears significantly undervalued or has strong momentum: **HOLD — DO NOT SELL CC**

## Early Exit Agent

Evaluate open positions for early closure when:
- 50–75% of premium captured
- Remaining premium small relative to capital/risk
- Risk/reward has deteriorated
- Stock moved substantially
- Volatility regime changed
- Better opportunities exist elsewhere
- Catalyst approaching

Do not hold simply because "premium remains."

## Bull Put Spread Alternative

When downside risk is elevated but bullish exposure is attractive, compare:

| Metric | CSP | Bull Put Spread |
|--------|-----|----------------|
| Premium | Higher | Lower |
| Max loss | Strike × 100 | Width × 100 |
| Capital efficiency | Lower | Higher |
| Assignment risk | Yes | No |

Recommend the spread when downside exposure reduction justifies lower premium.
...
**Action:** SELL CSP / SELL CC / HOLD / CLOSE / NO TRADE
**Confidence:** X/100
**Trade:** [TICKER] [EXPIRATION] [STRIKE] Put/Call
**Premium:** $X.XX ($XXX total)
**Break-even:** $XX.XX
**Capital required:** $X,XXX
**Maximum loss:** $X,XXX
**Expected return:** X.X% (XX% annualized)
**Delta:** 0.XX
**DTE:** XX days
**IV Rank:** XX%

**Why this trade (3–5 bullets):**
- [bullet 1]
- [bullet 2]
- [bullet 3]

**Main risks (3–5 bullets):**
- [bullet 1]
- [bullet 2]
- [bullet 3]
...
**Assignment plan:**
- If assigned: [action]
- CC strategy: [if applicable]
- Exit conditions: [when to close early]

**Adversarial conclusion:**
- [strongest argument against the trade]

**Alternative:**
- [best alternative trade or "cash is preferable because..."]

**Score:** XX/100
```

## Integration with PnL Report

The v3 report includes a new **Section 5: Trade Recommendations** after Section 4 (All Closed Trades):

### Section 5: Trade Recommendations

- **Market Context** — current regime assessment (bullish/neutral/bearish)
- **Portfolio Status** — current positions, cash deployed %, risk utilization
- **Recommended Trades** — ranked list of candidates that passed all agents
- **Rejected Candidates** — tickers evaluated but rejected, with reason
- **Risk Dashboard** — portfolio stress test results if recommendations are executed

## How to Run (v3 additions)
...
**Initially operates in PAPER / RECOMMENDATION MODE only.**

- Do not place real orders
- Present recommendations for human review
- Track paper performance against actual market outcomes
- Iterate on scoring weights based on outcome data

## Performance Tracking

For every recommended trade (whether taken or not), track:
- Entry conditions at time of recommendation
- What actually happened (stock price movement, option price, assignment)
- Outcome vs prediction
- Score calibration (did high-scored trades outperform?)

Measure against benchmarks:
- Buy-and-hold underlying
- SPY total return
- Cash/T-bill return
- Historical PnL per-trade average from v2 report

## Key Design Principle

> The system optimizes for **risk-adjusted total portfolio return**, NOT maximum option premium.
>
> A strategy that generates large premiums but suffers large losses on stock declines is NOT considered successful.
>
> Every trade must have: a clear reason for existing, a defined risk, and an explicit plan for adverse outcomes.
