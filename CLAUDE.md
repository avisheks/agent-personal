...
**Enforcement check:** Before running ANY Python script in `src/stock_research/`, generating ANY report file, or calling ANY data ingestion module, run: `grep "session_start" "$LOG_DIR/events.jsonl" 2>/dev/null | grep "$(date +%Y-%m-%d)"`. If this returns nothing, you skipped the orchestrator — STOP and route through `skills/super-agent.md` first. No exceptions.
