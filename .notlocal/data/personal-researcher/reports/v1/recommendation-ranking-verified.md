...
| 2. Identify constraints | Scale (millions of items), latency (100-300ms for LLM reranking), sparse feedback, cold-start, conflicting objectives | LLM-per-item is too expensive. Where does the LLM add value without blowing up cost? |
...
- Real-time ranking in the hot path can be achieved with latency < 50ms using specialized architectures and hardware
...
| Feature store | Feast, Tecton | Custom on Redis/DynamoDB | Managed offers scalability benefits for teams of all sizes |
...
|---------|---------------|-------------|-----------------|
| **Multi-stage pipeline** | Scale (can't score millions with one model) | Production systems that need to efficiently handle large and complex query spaces or item catalogs | Tiny catalogs (<1K) where you can score all |
...
| **Position bias correction (IPW)** | Debiased training from position-confounded clicks | Systems where position bias correction can be useful for debiasing training data, depending on specific characteristics | Position bias correction may be necessary even for systems with explicit ratings, as position bias can impact rating quality |
| **MMR diversity reranking** | Filter bubble prevention | User-facing surfaces where variety matters | B2B/transactional systems may still benefit from MMR diversity reranking to provide users with a range of relevant options |
...
| "We added an LLM to improve recommendations" | "We added LLM reranking selectively to high-value queries, complex natural language queries, and cold-start scenarios where behavioral data is insufficient. For 80% of queries, the classical pipeline outperforms at 1000x lower cost. The question isn't 'should we use LLMs' but 'where does the marginal value exceed the marginal cost?'" |
...
| "We improved NDCG by 5%" | "We improved NDCG by 5%, which translated to 2% CVR lift in the A/B test measured with double-randomized experimentation. Double-randomized experimentation addresses interference and provides accurate causal measurement in marketplace settings where standard A/B testing fails due to interference." |
...
### Online Learning
20. FTRL (Follow-The-Regularized-Leader) — online learning algorithm for real-time model updates
...
| Sections corrected | Design Flow Framework, Interview Q&A Bank, Full System Design Walkthrough (Principal/Director Level, ~4 min), Cost Model, Seniority Signals Cheat Sheet, Advanced Patterns Summary, References |
