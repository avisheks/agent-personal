...
This technical interview preparation guide covers the critical architectural decision between Supervised Fine-Tuning (SFT) and Direct Preference Optimization (DPO) in language model alignment systems. The report provides comprehensive coverage from foundational concepts through production deployment considerations, designed to prepare candidates for discussions ranging from L5 implementation details to Distinguished Engineer-level system architecture decisions. Whether you're optimizing for instruction-following capabilities or human preference alignment, this guide equips you with the frameworks, cost models, and observability patterns needed to design and defend SFT/DPO systems at scale.
...
|------|-------|---------------|
| 1. Clarify requirements | Define alignment objectives and success metrics | Choose between instruction-following, safety alignment, or domain-specific behavior; establish evaluation criteria (human preference scores, automated metrics, downstream task performance) |
| 2. Identify constraints | Assess data availability, compute budget, and organizational capabilities | Determine if you have demonstration data vs. preference pairs; evaluate RL infrastructure readiness; consider regulatory requirements for model behavior |
| 3. Propose baseline | Start with SFT using available demonstration data | Implement supervised fine-tuning with cross-entropy loss on (input, target_output) pairs; establish formatting and basic competence before preference optimization |
| 4. Identify gaps | Diagnose where imitation learning fails to capture preferences | Analyze cases where multiple valid responses exist; identify safety issues not covered by demonstrations; assess need for explicit preference modeling |
| 5. Introduce improvements | Layer preference optimization (DPO) or reward modeling (RLHF) | Choose DPO for stability and simplicity with pairwise data; select RLHF+PPO for complex reward structures; consider GRPO for reasoning tasks |
| 6. Add evaluation + guardrails | Implement comprehensive safety and quality monitoring | Deploy preference evaluation pipelines; add constitutional AI constraints; monitor for reward hacking and distribution shift |
| 7. Discuss scaling tradeoffs | Plan for 10x/100x model size and data volume increases | Address preference data collection bottlenecks; evaluate computational costs of different alignment methods; consider federated learning for sensitive domains |

### Decision Matrix

| Decision | Option A | Option B | Choose A when... | Choose B when... |
|----------|----------|----------|------------------|------------------|
| Training Method | Supervised Fine-Tuning (SFT) | Direct Preference Optimization (DPO) | You have high-quality demonstrations and need stable training | You have preference pairs and need to explicitly discourage bad outputs |
| Preference Learning | DPO (direct optimization) | RLHF with PPO (reward model + RL) | You want training stability and have pairwise preference data | You need complex reward structures or have scalar reward signals |
| Data Collection | Human demonstrations | Human preference comparisons | Experts can write good examples directly | It's easier to compare outputs than generate them |
| Evaluation Strategy | Automated metrics (BLEU, ROUGE) | Human preference evaluation | You have clear objective criteria and need fast iteration | Alignment quality matters more than evaluation speed |
| Scaling Approach | Parameter-efficient fine-tuning (LoRA) | Full model fine-tuning | Compute budget is limited or you need multiple task-specific variants | You have sufficient compute and need maximum performance |
