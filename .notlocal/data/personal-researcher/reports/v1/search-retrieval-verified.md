...
| 4. Identify gaps | Lexical fails on paraphrases, dense can struggle with exact matches, query ambiguity, lost intent in short queries | Systematic failure diagnosis: vocabulary mismatch vs intent mismatch vs coverage gap |
...
**Fusion methods**:
1. **RRF (Reciprocal Rank Fusion)**: RRF_score(d) = Σ_r 1/(k + rank_r(d)) where r iterates over all retrieval systems. No training needed. Robust. Standard choice.
...
The query understanding pipeline consists of multiple stages including spell correction, entity detection, and intent classification, and may involve additional steps such as query rewriting or expansion, depending on the specific application and system requirements:
...
|---------|---------------|-------------|-----------------|
| **Hybrid retrieval (BM25 + Dense + RRF)** | Vocabulary mismatch AND exact-match needs | Commonly used in production systems for balanced exact matching and semantic understanding | Tiny corpora (<1K) where brute-force works |
| **Cross-encoder reranking** | Fine-grained relevance precision | Quality-critical applications where additional latency is acceptable | Ultra-low latency (<20ms total budget) |
| **SPLADE (learned sparse)** | Semantic matching with inverted index efficiency | Can be considered as an alternative to hybrid retrieval systems where its strengths align with requirements | Cross-lingual retrieval; extremely short queries |
| **ColBERT (late interaction)** | Token-level matching faster than cross-encoder | Moderate latency budget; need interpretability | Memory-constrained (requires significantly more storage than bi-encoder) |
...
### Query Understanding
14. HyDE: Precise Zero-Shot Dense Retrieval without Relevance Labels (Gao et al., 2022) — HyDE generates a hypothetical ideal answer using a language model for retrieval purposes — https://arxiv.org/abs/2212.10496
...
| Sections corrected | Design Flow Framework, Full System Design Walkthrough (Principal/Director Level, ~4 min), Interview Q&A Bank, Seniority Signals Cheat Sheet, Advanced Patterns Summary, References |
