...
6. **Fine-tuned embeddings**: Train on domain-specific query-document pairs. Potentially significant improvement depending on factors like data quality and domain complexity, but higher effort [23].
...
8. **Contextual chunking**: Prepend document-level context to each chunk to make them self-contained and reduce fragmentation-induced hallucination.
9. **Hierarchical chunking**: Use different chunk sizes for retrieval versus generation to provide better context while maintaining precision.
...
The failure on exact terms happens due to a combination of factors including subword tokenization, frequency-based representation quality, and semantic smoothing, as well as the limitations of dense embeddings in capturing exact keyword matches:
...
- Cross-encoder: Must encode [query; doc_i] for each candidate at query time → requires K forward passes for K candidates, with each forward pass requiring significant computational resources due to the joint encoding of query-document pairs. Cannot precompute.
...
| **Contextual Retrieval** [3] | Chunk-boundary information loss | Contextual chunking addresses the issue of chunks losing meaning in isolation by prepending a context summary to each chunk |
| **Query Routing** [8] | One-size-doesn't-fit-all | Mixed query types (factual vs synthesis vs comparison) |
| **Hierarchical Retrieval** [7] | Precision vs context tradeoff | Hierarchical chunking allows for fine-grained retrieval with broader context for synthesis by maintaining parent-child relationships between chunks of different sizes |
...
|---|---|
| "We use vector search and BM25" | "Hybrid retrieval is highly beneficial for enterprise settings due to its ability to capture both exact keyword matches and semantic similarities, making it a preferred approach for many applications. The question is fusion strategy, not whether to combine." |
...
| Sections corrected | Interview Q&A Bank, Full System Design Walkthrough (Principal/Director Level, ~4 min), Advanced Patterns Summary, Distinguished Engineer Depth Probes, Seniority Signals Cheat Sheet |
