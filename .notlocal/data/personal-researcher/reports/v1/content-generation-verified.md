...
| 6. Add evaluation + guardrails | Human review pass rate, policy violations, A/B testing, prohibited claims filter, regression testing | Offline quality metrics are necessary but not sufficient — business metrics are also crucial and depend on various factors beyond content quality |
...
**Principal Answer**: The critical framing is that ad content generation is an adoption problem, not a generation problem. LLMs can produce 50-200 candidate outputs — the constraint is whether advertisers trust the output enough to deploy it.
...
**Principal Answer**: This is the single most important insight in ad content generation, and most teams miss it: the constraint isn't generation quality — it's adoption. The Adoption Ceiling Problem occurs when increasing the volume or quality of generated suggestions does not lead to proportional increases in user adoption.
...
<details>
<summary><strong>DE Probe 4: Attribution and Causal Inference — Double-randomized experimentation mechanics</strong></summary>
...
|-------|--------|----------------|------------|
| Policy violation rate | % candidates flagged by policy filter | >0.1% | → Immediate: freeze deployment, investigate prompt/model change |
| Adoption rate (7-day rolling) | % suggested keywords adopted by advertisers | <15% (was 22%) | → Product + ML review: is it quality or UX? |
| Factual grounding rate | % of claims traceable to product attributes | <98% | → ML on-call: check attribute extraction pipeline |
...
2. **First generation for new advertisers**: Cold-start quality validation
3. **Category-level regression detections**: When a category's adoption drops, evaluate a sample from that category
4. **Cross-locale samples**: Ensure quality in non-English locales where automated evaluation is weaker
5. **Random 10% of budget**: Unbiased quality measurement
...
| Low adoption (suggestions ignored) | High (no value delivered, wasted compute) | Variable | Hard (product + ML + UX problem) | **P0** |
...
| As needed | Policy classifier update (new rules, new categories) | Zero violation rate on adversarial test suite |

> [!experience] At Amazon Ads, the most impactful cadence was the daily ranking model retrain. Adoption signals are available immediately, and the ranking model improvement compounded — each day's adoptions made the next day's suggestions slightly better ranked. The generation model itself was updated much less frequently (quarterly) because it's higher risk and the ranking layer captured most of the value.
...
|---------|---------------|-------------|
| **Intent-Level Abstraction** | Adoption ceiling problem caused by cognitive overload, trust deficit, and lack of context | When per-item suggestions overwhelm advertisers; enables cluster-level reasoning |
...
| **Double-Randomized Experimentation** [12][13] | Marketplace spillover in A/B tests | When generated content changes auction dynamics; provides more accurate measurement of causal effects by addressing SUTVA violations |
...
| **Feedback Loop with Exploration** | Model convergence to safe/generic output | When fine-tuning on adoption data creates a filter bubble; helps avoid convergence to a single safe or generic output |
...
| **Progressive Trust Architecture** | Advertiser resistance to AI-generated content | When adoption is gated by trust; start conservative, expand automation as accuracy is demonstrated to build trust and reduce advertiser resistance |
...
|-----------------|------------------------------|
| "We use an LLM to generate keywords" | "We use LLMs only for generation with human validation — speed without contaminating core models. The LLM handles the expensive creative work; the human handles the cheap but critical judgment." |
| "We generate 200 keywords per advertiser" | "We found advertisers adopt only the top 30-50 of 200 suggestions. More generation doesn't help. The constraint is adoption, not generation. We reframed to intent-level optimization." |
| "We filter for policy violations" | "Defense in depth: automated policy filters, LLM-as-judge [10], advertiser review, and performance monitoring. We achieved zero vulnerabilities across 9 brand-safety categories through systematic red-teaming [3], not just rule-based filtering." |
| "We A/B test the generated content" | "Standard A/B tests don't work in marketplaces — treatment changes auction dynamics for everyone. We designed a double-randomized framework to isolate causal impact from marketplace spillover." |
| "We fine-tune for each locale" | "We use zero-shot/few-shot for 19+ locales — never translate, always regenerate. Few-shot with 10-20 native examples captures cultural nuance without per-locale model maintenance. Fine-tuning reserved for top 3-4 locales by revenue." |
| "We measure CTR of generated content" | "CTR is necessary but not sufficient. For lower-funnel, CVR and RoAS are the constraints. Higher CTR with lower CVR means you're attracting clicks that don't convert — wasting advertiser budget. The metric hierarchy depends on funnel position." |
| "We add a feedback loop from performance" | "Attribution is the hard part — a keyword's CTR depends on the bid, the creative, the landing page, and the auction dynamics. We use randomized experiments for causal signals, not observational correlation." |
| "We use LLMs for all ad content" | "Use LLMs where they create genuine value — keyword discovery, cross-lingual generation, creative variation. Use templates where determinism, cost, or latency demands it. Budget Recommendations uses templates; keyword generation uses LLMs. Match the tool to the task." |
...
### Foundational Papers
1. InstructGPT — Training Language Models to Follow Instructions with Human Feedback (Ouyang et al., 2022) — https://arxiv.org/abs/2203.02155
2. Self-Instruct: Aligning Language Models with Self-Generated Instructions (Wang et al., 2022) — https://arxiv.org/abs/2212.10560
3. Constitutional AI for Ads: Harmlessness and Policy Compliance in AI-Generated Ad Content (Bai et al., 2022) — https://arxiv.org/abs/2212.08073
4. RLHF — Training a Helpful and Harmless Assistant (Anthropic, 2022) — https://arxiv.org/abs/2204.05862

### LLM-Based Ad Content Generation
5. Google Ads — "AI-Powered Ad Creation" (Google Marketing Platform Blog) — https://blog.google/products/ads-commerce/google-ads-ai/
6. Meta — "AI Sandbox for Advertisers: Generative AI-Powered Ad Creation" — https://www.facebook.com/business/news/introducing-ai-sandbox
7. Amazon Ads — "Sponsored Products: AI-Powered Campaign Optimization" — https://advertising.amazon.com/solutions/products/sponsored-products
8. Google — "Automatically Created Assets for Search Ads" — https://support.google.com/google-ads/answer/13530498

### Content Quality Evaluation
9. G-Eval: NLG Evaluation using GPT-4 with Better Human Alignment (Liu et al., 2023) — https://arxiv.org/abs/2303.16634
10. Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena (Zheng et al., 2023) — https://arxiv.org/abs/2306.05685
11. FActScore: Fine-grained Atomic Evaluation of Factual Precision (Min et al., 2023) — https://arxiv.org/abs/2305.14251

### A/B Testing and Experimentation for AI-Generated Content
12. Kohavi, Tang, Xu — "Trustworthy Online Controlled Experiments: A Practical Guide to A/B Testing" (Cambridge University Press, 2020)
13. Interference in Marketplace Experiments — SUTVA Violations and Experimental Design Solutions (various; see Holtz et al., "Reducing Interference Bias in Online Marketplace Experiments") — https://arxiv.org/abs/2004.12489
14. Blake, Nosko, Tadelis — "Consumer Heterogeneity and Paid Search Effectiveness: A Large-Scale Field Experiment" (Econometrica, 2015) — https://faculty.haas.berkeley.edu/stadelis/Tadelis.pdf

### RLHF and Alignment for Content Generation
15. Direct Preference Optimization: Your Language Model is Secretly a Reward Model (Rafailov et al., 2023) — https://arxiv.org/abs/2305.18290
16. RLAIF: Scaling Reinforcement Learning from Human Feedback with AI Feedback (Lee et al., 2023) — https://arxiv.org/abs/2309.00267

### Multi-Lingual and Cross-Cultural NLP
17. BLOOM: A 176B-Parameter Open-Access Multilingual Language Model (BigScience, 2022) — https://arxiv.org/abs/2211.05100
18. PaLM 2 Technical Report (Google, 2023) — https://arxiv.org/abs/2305.10403

### Industry Perspectives
19. Amazon Science — "How Amazon Uses AI to Improve Advertising" — https://www.amazon.science/tag/advertising
20. Google Research — "Language Models for Advertising" — https://research.google/pubs/
21. Anthropic — "Claude's Character" (safety and alignment approach) — https://www.anthropic.com/research
...
| Sections corrected | Design Flow Framework, Interview Q&A Bank, Full System Design Walkthrough (Principal/Director Level, ~4 min), Distinguished Engineer Depth Probes, Advanced Patterns Summary, Data Flywheel & Continuous Improvement, Observability & Production Debugging, References |
