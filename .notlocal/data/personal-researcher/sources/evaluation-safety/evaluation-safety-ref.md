---
title: "Evaluation Safety Ref"
source: "data/researcher/reports/evaluation-safety-ref.md"
ingestedAt: "2026-05-17T14:56:49Z"
---
...
| 6. Add evaluation of the evaluator | Meta-evaluation: evaluating the evaluator itself, drift detection, inter-judge agreement, adversarial testing of the eval system | Who watches the watchmen? |
...
"Evaluation is the highest-leverage platform capability in any AI org — not research overhead. Without a credible evaluation gate, you have two failure modes: you ship dangerous models because you can't detect problems, or you never ship because no one trusts the metrics enough to approve a release. From my experience at Amazon Ads, I designed the five-layer evaluation framework that became the standard release gate for every foundation model and agent we shipped. The key insight: evaluation quality depends on DECOMPOSITION, not single metrics. You can't ask 'is this model good?' — you must decompose into 'is it safe?', 'is it accurate?', 'is it useful?', and evaluate each independently."
### 1. Clarify Requirements

Before designing anything, I'd ask:

- **What's being evaluated?**: A foundation model (text generation quality)? An agent (multi-step task completion)? A RAG pipeline (retrieval + generation)? A content moderation system (safety classification)? Each requires fundamentally different evaluation approaches.
- **What's the release bar?**: Zero tolerance for safety failures? 95%+ on task completion? "Better than the previous version" (relative) or "meets absolute threshold" (absolute)?
- **Who are the stakeholders?**: Scientists want model quality metrics. Product wants business impact. Legal/policy wants safety compliance. Leadership wants confidence that it won't embarrass the company. Each stakeholder needs different evidence.
- **What's the cost of a bad release?**: A hallucinating chatbot loses user trust. A hallucinating ad content system creates legal liability. A hallucinating medical system kills people. Evaluation rigor must match risk.
- **Evaluation cadence**: One-time release gate? Continuous monitoring? Both? Models drift, data changes, and what passed evaluation last month may fail today.
- **Evaluation latency budget**: Can we spend 4 hours evaluating before a release? Or does CI/CD require evaluation in <15 minutes?

**Principal signal**: Frame evaluation as a business risk management problem, not a model quality measurement problem. "The evaluation system exists to give leadership decision-grade confidence before exposing customers to a new model."

> [!experience] At Amazon Ads, the business problem was trust at release time. Without a credible gate, every new FM or agent improvement risked shipping hidden regressions into a revenue-critical, brand-sensitive workflow. Leadership lacked confidence that "public benchmark gains" translated to advertiser-safe behavior.

### 2. Identify Constraints

- **SME availability**: Human experts are expensive, slow, and inconsistent. You can't scale human review to every release candidate. But automation alone isn't trusted.
- **Evaluation-velocity tradeoff**: Thorough evaluation takes days. Shipping takes hours. Teams that can't evaluate fast enough stop shipping — evaluation becomes a bottleneck, not a gate.
- **Domain specificity**: Public benchmarks (MMLU, HumanEval, HELM) measure generic capabilities. They tell you nothing about whether the model handles YOUR domain (ads policy, medical compliance, legal reasoning) correctly.
- **Agentic evaluation complexity**: For agents, you can't just evaluate outputs — you must evaluate trajectories (multi-step sequences). A model evaluation asks "is this text correct?" An agent evaluation asks "was this SEQUENCE of actions correct AND safe AND complete?"
- **Organizational trust**: Even if your evaluation framework is rigorous, it must be BELIEVED by stakeholders. A framework that scientists trust but product managers don't is useless — they'll block releases anyway.
- **Moving targets**: Models improve, data drifts, policies change. An evaluation suite built in January may miss failures that only emerge in March. Evaluation must evolve continuously.

**Risk framing**: (P0) Business: bad release harms customers/advertisers, damages brand | (P1) Technical: evaluation itself can be wrong (false confidence, scoring drift) | (P2) Org: teams don't trust eval → either ship without eval (dangerous) or block everything (slow)

### 3. Propose Baseline (Benchmarks + Manual Review)

**Architecture:**

```
┌──────────────────────────────────────────────────────────────────────────────────┐
│  OFFLINE EVALUATION PIPELINE (Baseline)                                           │
├──────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│  ┌─────────────┐    ┌──────────────────┐    ┌─────────────────┐                 │
│  │ Model Under │───▶│ Public Benchmark │───▶│ Score Report    │                 │
│  │ Test (MUT)  │    │ Suite (MMLU,     │    │ (vs baseline,   │                 │
│  │             │    │ HumanEval, etc.) │    │  vs previous)   │                 │
│  └─────────────┘    └──────────────────┘    └─────────────────┘                 │
│         │                                                                         │
│         │           ┌──────────────────┐    ┌─────────────────┐                 │
│         └──────────▶│ Manual SME       │───▶│ Pass/Fail       │                 │
│                     │ Review (sample   │    │ Decision        │                 │
│                     │ of 50-100 cases) │    │                 │                 │
│                     └──────────────────┘    └─────────────────┘                 │
│                                                                                   │
└──────────────────────────────────────────────────────────────────────────────────┘
...
**Components:**
- **Public benchmarks**: MMLU (knowledge), HumanEval (code), MT-Bench (conversation), HELM (holistic). Quick to run, reproducible, but don't measure domain-specific behavior.
- **Manual SME review**: Domain experts evaluate 50-100 model outputs on a rubric (accuracy, safety, usefulness). High quality but expensive, slow, and inconsistent between reviewers.
- **Release decision**: Simple threshold — benchmark scores must meet or exceed previous version AND SME pass rate must be >90%.

**Why start here**: This establishes the evaluation habit and gives stakeholders SOMETHING to reference. It's credible enough to gate early releases while you build the real system.

**Design choice**: Start with benchmarks + manual review over building a custom automated system first
- **Pros**: Ships immediately, builds organizational trust in "evaluation as a gate," surfaces what metrics stakeholders actually care about
- **Cons**: Doesn't scale, SME reviews are inconsistent, benchmarks miss domain-specific failures, no coverage of agent trajectories
- **Why chosen** (working backward from requirements): You need to gate releases NOW while building the real evaluation system. Perfect is the enemy of shipped.
- **Alternative considered**: Build full automated evaluation first — takes months, during which models ship unvalidated

### 4. Identify Gaps (Where Baseline Fails)

| Failure Mode | Symptom | Root Cause |
|---|---|---|
| **Benchmark-reality gap** | Model scores 90% on MMLU but fails on domain tasks | Public benchmarks don't cover your domain (ads policy, product knowledge, locale-specific rules) |
| **SME inconsistency** | Two reviewers give opposite scores on the same output | No calibrated rubric; reviewers interpret quality differently |
| **SME doesn't scale** | Evaluation backlog of 2 weeks; teams stop waiting and ship without eval | 50 outputs × 30 minutes/review = 25 hours per release |
| **Safety gaps** | Model passes all benchmarks but generates policy-violating content in production | Benchmarks don't test adversarial inputs, edge cases, or domain-specific policy |
| **No trajectory evaluation** | Agent completes task but via unsafe intermediate steps | Evaluation only checks final output, not the path |
| **No regression detection** | New model is better overall but worse on specific critical slice | Aggregate metrics hide per-segment regressions |
| **Evaluation drift** | Evaluation suite becomes stale; new failure modes emerge that aren't tested | Eval set is static while the model and world change |
| **Scoring drift in LLM-judges** | LLM-based evaluator's scores shift over time (model updates change judge behavior) | Judge model is updated but evaluation thresholds aren't recalibrated |

> [!experience] At Amazon Ads, public benchmarks were necessary but not sufficient because they optimize for the wrong objective. In ads workflows, the system must be safe, compliant, and operationally useful under Amazon-specific constraints — none of which public benchmarks measure. We also found that SME disagreement exposed hidden policy ambiguity — reviewers disagreed not because they were wrong, but because the policy itself was unclear. Evaluation surfaced organizational problems, not just model problems.

### 5. Introduce Improvements

#### 5a. Layered Evaluation Stack (Five Layers)

The core architecture: each layer catches a different class of failure, with increasing semantic depth and cost.
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Layer 1: Deterministic Tests [<1s, $0]                                  │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Format validation, regex safety checks, length limits,           │    │
│  │ prohibited term detection, schema compliance                     │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│           │ (pass: 98%+ of outputs)                                      │
│           ▼                                                              │
│  Layer 2: Automated Benchmarks [minutes, ~$0]                            │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Domain-specific test suites (not public benchmarks), regression   │    │
│  │ tests from past failures, golden-set accuracy                    │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│           │ (pass: 90%+ accuracy on golden set)                          │
│           ▼                                                              │
│  Layer 3: LLM-as-Judge [minutes, ~$1-5/eval run]                         │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Calibrated LLM judge scores on rubric dimensions (accuracy,      │    │
│  │ safety, usefulness, policy compliance). Cross-family judging.    │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│           │ (pass: mean score > threshold per dimension)                  │
│           ▼                                                              │
│  Layer 4: SME Review [hours-days, $$$]                                   │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Human experts review sample (stratified by risk, difficulty).     │    │
│  │ Calibrated rubric with inter-rater agreement tracking.           │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│           │ (pass: >85% SME approval rate)                               │
│           ▼                                                              │
│  Layer 5: UAT / Shadow Production [days-weeks, operational cost]         │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Model runs in shadow mode on production traffic. Compare outputs │    │
│  │ against current production model. A/B test on live users.        │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│           │ (pass: no regression on key business metrics)                 │
│           ▼                                                              │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    RELEASE DECISION                               │    │
│  └─────────────────────────────────────────────────────────────────┘    │
...
> [!experience] At Amazon Ads, I designed this exact five-layer stack. The key insight: early automated filters catch regressions cheaply (Layer 1-2 catch 70% of issues at near-zero cost). LLM-judge catches nuanced quality issues (Layer 3 catches 20% at moderate cost). SME review catches the remaining edge cases (Layer 4 catches 8% at high cost). UAT validates that lab quality translates to production quality (Layer 5 catches 2% that only appear under real traffic). Each layer has diminishing returns but catches failures the previous layers missed.

**Risk framing**: (P0) Business: shipping a model that fails on safety/policy → advertiser harm, regulatory risk | (P1) Technical: LLM-judge drift means Layer 3 passes things it shouldn't → silent degradation | (P2) Org: if evaluation is too slow, teams bypass it → the gate becomes theater

#### 5b. LLM-as-Judge with Calibration

Using one LLM to judge another's outputs. Powerful but requires careful calibration:

**Known biases**:
- **Position bias**: First response in a pairwise comparison gets higher scores
- **Self-preference**: Models prefer outputs from their own family (GPT judges prefer GPT outputs)
- **Verbosity bias**: Longer outputs score higher regardless of quality
- **Sycophancy**: Judge follows the prompt's implied preference rather than independent assessment

**Calibration protocol**:
1. **Cross-family judging**: If your model is GPT-based, judge with Claude (and vice versa). Reduces self-preference.
2. **Position randomization**: Randomize order in pairwise comparisons. Average across both orderings.
3. **Calibration set**: 200+ examples scored by both LLM-judge AND human experts. Measure agreement (Cohen's κ). If κ < 0.7, the judge needs recalibration.
4. **Drift monitoring**: Re-run calibration set monthly. If agreement drops, the judge or the domain has shifted.
5. **Rubric anchoring**: Don't ask "is this good?" Ask "does this output satisfy criterion X? Evidence: [specific text from output]." Structured rubrics reduce judge variance.

> [!experience] At Amazon Ads, we calibrated LLM-judges with 1P/3P SMEs to reduce drift. High inter-judge variance forced us to implement disagreement tracking — when the LLM judge and human experts disagreed, we investigated: was the judge wrong, or was the policy ambiguous? Often it was the latter, which surfaced organizational clarity issues that needed resolution before evaluation could be reliable.

**Design choice**: LLM-as-judge with calibration over pure human evaluation
- **Pros**: 100x faster than human review, consistent (no reviewer fatigue), scales to thousands of outputs, available 24/7
- **Cons**: Inherits biases, can drift silently, can't evaluate truly novel situations, not trusted by all stakeholders initially
- **Why chosen** (working backward from requirements): Human review doesn't scale to every release. LLM-judge handles 80% of evaluation volume; humans handle the critical 20% (safety-sensitive, high-stakes, edge cases).
- **Alternative considered**: Pure human review — highest quality per-eval but creates 2-week backlogs that block shipping

#### 5c. Rubric Design

The rubric is the most important artifact in the evaluation system — it defines what "good" means, consistently, across evaluators (human and LLM).

**Principles**:
1. **Decompose quality into dimensions**: Don't ask "is this output good?" Ask: "Is it accurate? (score 1-5) Is it safe? (score 1-5) Is it useful? (score 1-5) Is it policy-compliant? (binary)" Each dimension is evaluated independently.
2. **Anchor with examples**: For each score level (1-5), provide a concrete example of what that score looks like. "A 3 on accuracy means: mostly correct but contains one minor factual error that doesn't change the overall message."
3. **Binary for safety**: Safety is not a spectrum. "Does this output contain a policy violation? Yes/No." Partial credit on safety is dangerous.
4. **Dimension weights by use case**: For ads content, policy compliance is weighted highest (one violation = fail). For internal analytics, accuracy is weighted highest.

#### 5d. Segment-Aware Evaluation

Aggregate metrics hide critical failures. A model that scores 95% overall may score 60% on a critical segment (e.g., medical claims in ads, high-spend advertisers, non-English locales).

**Mandatory slicing dimensions**:
- By content type (factual vs creative vs policy-sensitive)
- By risk level (high-stakes vs low-stakes)
- By locale/language
- By user segment (new vs experienced, small vs large advertisers)
- By query complexity (simple vs multi-hop)

**Rule**: A model cannot pass evaluation if ANY critical segment regresses, even if the aggregate improves. "Better on average but worse for our largest advertisers" is not acceptable.

#### 5e. Trajectory Evaluation (for Agents)

Standard model evaluation scores individual outputs. Agent evaluation must score multi-step trajectories:

1. **Task decomposition**: Break the trajectory into discrete steps
2. **Per-step scoring**: Was each step correct? Right tool? Right parameters? Right interpretation?
3. **Trajectory scoring**: Was the overall path efficient? Safe? Did errors compound?
4. **Counterfactual assessment**: Was there a better path the agent should have taken?

> [!experience] At Amazon Ads, agentic extensions introduced emergent, multi-turn failure modes that static metrics missed. An agent could produce a "correct" final recommendation via a dangerous path — accessing data it shouldn't have, making unnecessary tool calls, or building on a hallucinated intermediate result that happened to lead to the right answer. Trajectory evaluation catches these "correct but unsafe" paths that output-only evaluation misses entirely.

#### 5f. Continuous Evaluation (not just release gates)

Evaluation doesn't stop at release. Production models drift, data changes, and new failure modes emerge:

- **Continuous monitoring**: Run a subset of the eval suite against production outputs daily. Alert on regression.
- **Canary evaluation**: Before rolling out to all users, evaluate on 1% of traffic with full eval suite.
- **User-signal integration**: Thumbs down, regeneration requests, and support tickets are evaluation signals. Feed them back.
- **Eval set expansion**: Every production failure becomes a test case. The eval suite grows organically from real failures.

### 6. Evaluation of the Evaluator (Meta-Evaluation)

#### Who Watches the Watchmen?

The evaluation system itself can fail:
- LLM-judge drifts silently (model update changes scoring behavior)
- Eval set becomes stale (doesn't cover new failure modes)
- Rubric has blind spots (doesn't test what matters)
- False confidence: evaluation says "pass" but production quality degrades

**Meta-evaluation framework**:

1. **Judge calibration tracking**: Monthly re-run of calibration set. Plot agreement over time. Alert if κ drops.
2. **Eval-production correlation**: Track correlation between eval scores and production metrics (user feedback, error rates). If they diverge, evaluation is measuring the wrong thing.
3. **Adversarial testing of the eval system**: Deliberately submit outputs with known flaws. Does the evaluation catch them? Red-team your own eval.
4. **Eval set coverage analysis**: What percentage of production failure modes are covered by the eval suite? Track coverage gaps.
5. **Inter-rater reliability**: For human evaluation, track agreement between reviewers. Low agreement = rubric problem or policy ambiguity.

> [!experience] At Amazon Ads, we discovered that noisy agent traces delayed root-cause analysis until observability improved. Similarly, our evaluation system had a blind spot: it was strong for structured reasoning (multiple-choice, factual) but weak for long-horizon strategy tasks (open-ended, multi-step). Meta-evaluation surfaced this gap — production failures in strategy recommendations weren't being caught because the eval suite didn't have strategy test cases. We added them.

### 7. Scaling Tradeoffs

#### Evaluation Rigor vs Shipping Speed

| Release Type | Eval Rigor | Latency | When |
|---|---|---|---|
| Hotfix (safety patch) | Layer 1 only (deterministic) | <15 min | Active harm in production |
| Minor update (prompt change) | Layers 1-3 (automated) | <2 hours | Low-risk change, well-understood |
| Model version update | Layers 1-4 (full suite) | 1-3 days | New model version, moderate risk |
| New capability launch | Layers 1-5 (full + UAT) | 1-2 weeks | New feature, unknown risk profile |

**The Director's decision**: Match evaluation rigor to release risk. Over-evaluating low-risk changes kills velocity. Under-evaluating high-risk changes kills users.

#### Human-in-the-Loop Balance

- **100% human review**: Highest quality, completely unscalable (50 outputs × 30 min = 25 hours per release)
- **0% human review**: Fastest, but no stakeholder trusts it (especially for safety)
- **Optimal**: LLM-judge for volume, human for critical slice (safety-sensitive, high-stakes, novel cases). Typically 80% automated / 20% human.

#### Cost of Evaluation vs Cost of Bad Release

| Evaluation Level | Cost | What It Catches | Miss Rate |
|---|---|---|---|
| Layers 1-2 only | ~$0 + 30 min | Format errors, known regressions | Misses 30% of issues |
| Layers 1-3 | ~$5 + 2 hours | Quality issues, policy violations | Misses 10% of issues |
| Layers 1-4 | ~$500 + 2 days | Edge cases, subtle safety issues | Misses 2% of issues |
| Layers 1-5 | ~$5000 + 2 weeks | Production-only failures | Misses <0.5% of issues |

**Frame the tradeoff**: "A bad release that creates a policy violation costs us $X in legal + brand damage + advertiser churn. Evaluation that catches it costs $Y. As long as X >> Y, invest in evaluation."

#### Reframing Evaluation as Platform

> [!experience] At Amazon Ads, the most important reframing was: evaluation is a PLATFORM CAPABILITY, not research overhead. When I positioned it as a platform, it got engineering investment, SLA requirements, and organizational ownership. When it was "the eval team's research project," it was underfunded and nobody trusted it enough to block a release on its results.
[[#Evaluation + Safety System Design — Interview Prep|↑ Top]]
...
- κ 0.61-0.80: Substantial (minimum for evaluation gating)
...
**Answer**: (1) After any judge model update — mandatory. (2) Monthly on a stable judge — detect drift early. (3) After any rubric change — the scoring standard changed, so the calibration is invalidated. Track the "recalibration delta" (how much the correction function changed) as a health metric. Large deltas = the judge is unstable and may need replacement rather than recalibration.
...
False Positive Rate = benign_inputs_blocked / total_benign_inputs
  Target: < 0.5% (don't block legitimate users)
```
...
| Layer 2: Domain benchmarks | ~$0.50 | 500 test cases × embedding + inference | Cache embeddings; parallelize |
| Layer 3: LLM-as-judge | ~$5-15 | 500 outputs × ~500 tokens each scored by Sonnet | Haiku for simple dimensions; batch scoring |
| Layer 4: Human SME review | ~$200-500 | 50-100 outputs × $3-5 per output (annotator cost) | Stratified sampling (only review high-risk slice) |
...
|-------|--------|----------------|------------|
| Judge calibration κ | Cohen's κ vs human gold labels | <0.70 (was 0.80) | → Eval team: recalibrate or replace judge |
...
| Judge calibration κ decline | Monthly | Monthly | Judge is drifting | Recalibrate or replace judge |
...
|---------|---------------|-------------|-----------------|
| **Layered evaluation (5-layer stack)** | Single-metric blindness; each layer catches different failure class | Any production LLM/agent system | Prototypes (overkill — use Layer 1-2 only) |
...
| **Cross-family judging** | Self-preference bias in LLM judges | Always (when using LLM-as-judge) | If only one model family is available |
...
| **Adversarial red-teaming** | Unknown safety vulnerabilities | Before any customer-facing launch | Internal-only tools with no safety risk |
...
| "We use GPT-4 as a judge" | "We use cross-family judging calibrated monthly against human experts. Self-preference bias inflated scores by 0.5 points until we switched to cross-family. We track Cohen's κ and alert if it drops below 0.75." |
...
19. Inter-Rater Reliability: Cohen's Kappa (McHugh, 2012) — https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3900052/
20. Krippendorff's Alpha (Hayes & Krippendorff, 2007) — standard reference for multi-rater agreement
